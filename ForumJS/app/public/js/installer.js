...
(function($) { // Avoid conflicts with other libraries
	...
	function parseMessage(messageJSON) {
		...
		try {
			responseObject = messageJSON;//JSON.parse(messageJSON);
		} catch (err) {
			if (window.console) {
	...
	function queryInstallerStatus() {
		...
		url = url.substring(0, position) + lookUp + '/install.html?action=status';
	...
	function pollContent(xhReq) {
		var messages = JSON.parse(xhReq.responseText).messages;
...
*/
	...
	function doRefresh() {
		...
		var xhReq = createXhrObject();
		xhReq.open('GET', $(location).attr('pathname')+'?action=status', true);
		xhReq.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
		xhReq.send();
